package hip.ch4.hbase;

import hip.ch3.hbase.HBaseWriter;
...
public class HBaseSinkMapReduce extends Configured implements Tool {
  ...
  public int run(final String[] args) throws Exception {
    ...
    Cli cli = Cli.builder().setArgs(args).addOptions(CliCommonOpts.IODirOpts.values()).build();
    ...
    Path inputPath = new Path(cli.getArgValueAsString(CliCommonOpts.IODirOpts.INPUT));
    Path outputPath = new Path(cli.getArgValueAsString(CliCommonOpts.IODirOpts.OUTPUT));
